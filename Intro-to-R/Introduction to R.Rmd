...
In the class directory (Intro-to-R), double-click on the `Intro-to-R.Rproj` file and it will open in RStudio. 

RStudio is your dashboard for writing R scripts. We encourage the use of __Notebook__ files (.Rmd), which enable you to both write comments (like what I'm writing here) and code, in what we call a __code chunk__:
```{r}
x <- "Hello, World!"
print(x)
```
...
Script files show up in the upper-left quadrant of RStudio. In the lower-left, you'll see the __Console__. This will reflect all the R code you run, but we don't do much with it when we're working in R Notebook files. 

The upper-right is your __environment__: if you store any information in variables (such as a table of data), they will show up there. 

The lower-right quadrant has several tabs, the first of which is Files: it shows what files exist in your current working directory. For this project, that should be the `Intro-to-R` folder which has the `Intro-to-R.Rproj` file we clicked on. 
...
Using the Files pane in the lower-right quadrant, click once on the filename `R-101.Rmd`. It should open in the upper-left quadrant. 
