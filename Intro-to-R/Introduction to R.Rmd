...
In the class directory (Intro-to-R), double-click on the `Intro-to-R.Rproj` file and it will open in RStudio: ![RStudio screenshot](images/Rstudio.png) 
...
The lower-right quadrant also has several tabs, the first of which is Files: it shows what files exist in your current working directory. For this project, that should be the `Intro-to-R` folder which has the `Intro-to-R.Rproj` file we clicked on. Go ahead and click once on the `R-101.Rmd` file, which is an R Notebook file. It will appear in the upper-left quadrant.
